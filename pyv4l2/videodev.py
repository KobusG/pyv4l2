...
class VideoDevice:
    def __init__(self, dev_path: str) -> None:
        ...
        cap = uapi.v4l2_capability()
        fcntl.ioctl(self.fd, uapi.VIDIOC_QUERYCAP, cap, True)

        if cap.device_caps & uapi.V4L2_CAP_VIDEO_CAPTURE_MPLANE:
            self.has_capture = True
            self.has_mplane_capture = True
        elif cap.device_caps & uapi.V4L2_CAP_VIDEO_CAPTURE:
            self.has_capture = True
        ...
        if cap.device_caps & uapi.V4L2_CAP_VIDEO_OUTPUT_MPLANE:
            self.has_output = True
            self.has_mplane_output = True
        elif cap.device_caps & uapi.V4L2_CAP_VIDEO_OUTPUT:
            self.has_output = True
        ...
        if cap.device_caps & uapi.V4L2_CAP_VIDEO_M2M_MPLANE:
            self.has_m2m = True
            self.has_capture = True
            self.has_output = True
            ...
            self.has_mplane_capture = True
            self.has_mplane_output = True
        elif cap.device_caps & uapi.V4L2_CAP_VIDEO_M2M:
            self.has_m2m = True
            self.has_capture = True
            self.has_output = True
        ...
        if cap.device_caps & uapi.V4L2_CAP_META_CAPTURE:
            self.has_meta_capture = True
        else:
            self.has_meta_capture = False

        if cap.device_caps & uapi.V4L2_CAP_META_OUTPUT:
    ...
    def find_video_device(key: str, value: str) -> str:
        for path in glob.glob('/dev/video*'):
            ...
            try:
                cap = uapi.v4l2_capability()
                fcntl.ioctl(fd, uapi.VIDIOC_QUERYCAP, cap, True)
    ...
    def get_formats(self, buf_type: pyv4l2.BufType):
        fmt = uapi.v4l2_fmtdesc()
        ...
        while True:
            try:
                fcntl.ioctl(self.fd, uapi.VIDIOC_ENUM_FMT, fmt, True)
            except OSError as e:
            ...
            try:
                if buf_type in [pyv4l2.BufType.META_CAPTURE, pyv4l2.BufType.META_OUTPUT]:
                    f = pyv4l2.MetaFormats.find_v4l2_fourcc(fmt.pixelformat)
                else:
                    f = pyv4l2.PixelFormats.find_v4l2_fourcc(fmt.pixelformat)
                fmts.append(f)
    ...
    def get_unsupported_formats(self, buf_type: pyv4l2.BufType) -> list[str]:
        fmt = uapi.v4l2_fmtdesc()
        fmt.type = buf_type.value
        fmt.index = 0
        ...
        while True:
            ...
            try:
                if buf_type in [pyv4l2.BufType.META_CAPTURE, pyv4l2.BufType.META_OUTPUT]:
                    pyv4l2.MetaFormats.find_v4l2_fourcc(fmt.pixelformat)
                else:
                    pyv4l2.PixelFormats.find_v4l2_fourcc(fmt.pixelformat)
            except StopIteration:
                fmts.append(pyv4l2.fourcc_to_str(fmt.pixelformat))

            fmt.index += 1

        return fmts

    def get_format(self, buf_type: pyv4l2.BufType):
        fmt = uapi.v4l2_format()
        fmt.type = buf_type.value
        fcntl.ioctl(self.fd, uapi.VIDIOC_G_FMT, fmt, True)
        return fmt

    def get_capture_streamer(self, mem_type: pyv4l2.MemType,
                             width: int, height: int, format: pyv4l2.PixelFormat):
        if not self.has_capture:
            raise NotImplementedError()

        if self.has_mplane_capture:
            return MPlaneCaptureStreamer(self, mem_type, pyv4l2.BufType.VIDEO_CAPTURE_MPLANE,
                                         width, height, format)

        return SPlaneCaptureStreamer(self, mem_type, pyv4l2.BufType.VIDEO_CAPTURE,
                               width, height, format)

    def get_meta_capture_streamer(self, mem_type: pyv4l2.MemType,
                                  size: int | tuple[int, int],
                                  format: pyv4l2.MetaFormat):
        if self.has_meta_capture:
            return MetaCaptureStreamer(self, mem_type, pyv4l2.BufType.META_CAPTURE,
                                       size, format)

        if self.has_meta_output:
            assert isinstance(size, int)

            return MetaOutputStreamer(self, mem_type, pyv4l2.BufType.META_OUTPUT,
                                       size, format)
...
class CaptureStreamer(ABC):
    def __init__(self, vdev: VideoDevice, mem_type: pyv4l2.MemType, buf_type: pyv4l2.BufType) -> None:
        self.vdev = vdev
    ...
    def set_queue_size(self, queue_size):
        v4lreqbuf = uapi.v4l2_requestbuffers()
        v4lreqbuf.type = self.buf_type.value
        v4lreqbuf.memory = self.mem_type.value
        v4lreqbuf.count = queue_size
        fcntl.ioctl(self.fd, uapi.VIDIOC_REQBUFS, v4lreqbuf, True)
        assert(v4lreqbuf.count == queue_size)

    def stream_on(self):
        buf_type = ctypes.c_uint32(self.buf_type.value)
        fcntl.ioctl(self.fd, uapi.VIDIOC_STREAMON, buf_type, True)

    def stream_off(self):
        buf_type = ctypes.c_uint32(self.buf_type.value)
        fcntl.ioctl(self.fd, uapi.VIDIOC_STREAMOFF, buf_type, True)
    ...
    @property
    @abstractmethod
    def format(self) -> pyv4l2.PixelFormat | pyv4l2.MetaFormat: ...
...
class VideoCaptureStreamer(CaptureStreamer):
    def __init__(self, vdev: VideoDevice, mem_type: pyv4l2.MemType, buf_type: pyv4l2.BufType,
                 width: int, height: int, format: pyv4l2.PixelFormat) -> None:
    ...
    @property
    def format(self) -> pyv4l2.PixelFormat:
    ...
    def reserve_buffers(self, num_bufs):
        ...
        for i in range(num_bufs):
            vbuf = VideoBuffer(pyv4l2.MemType.MMAP, i)
    ...
    def reserve_buffers_dmabuf(self, dmabuf_fds: list[int]):
        ...
        for i,fd in enumerate(dmabuf_fds):
            vbuf = VideoBuffer(pyv4l2.MemType.DMABUF, i)
            vbuf.fd = fd
            self.vbuffers.append(vbuf)
...
class SPlaneCaptureStreamer(VideoCaptureStreamer):
    ...
    def set_format(self):
        v4lfmt = uapi.v4l2_format()

        v4lfmt.type = self.buf_type.value
        fcntl.ioctl(self.fd, uapi.VIDIOC_G_FMT, v4lfmt, True)
        ...
        pix.field = uapi.V4L2_FIELD_NONE

        fcntl.ioctl(self.fd, uapi.VIDIOC_S_FMT, v4lfmt, True)
    ...
    def queue(self, vbuf: VideoBuffer):
        ...
        v4l2buf = uapi.v4l2_buffer()
        v4l2buf.type = self.buf_type.value
        v4l2buf.memory = vbuf.mem_type.value
        v4l2buf.index = vbuf.index
        if vbuf.mem_type == pyv4l2.MemType.DMABUF:
            v4l2buf.m.fd = vbuf.fd

        fcntl.ioctl(self.fd, uapi.VIDIOC_QBUF, v4l2buf, True)
    ...
    def dequeue(self) -> VideoBuffer:
        ...
        fcntl.ioctl(self.fd, uapi.VIDIOC_DQBUF, v4l2buf, True)
        ...
        if self.mem_type == pyv4l2.MemType.DMABUF:
...
class MPlaneCaptureStreamer(VideoCaptureStreamer):
    ...
    def queue(self, vbuf: VideoBuffer):
        ...
        planes = (uapi.v4l2_plane * num_planes)()
...
class MetaCaptureStreamer(CaptureStreamer):
    def __init__(self, vdev: VideoDevice, mem_type: pyv4l2.MemType, buf_type: pyv4l2.BufType,
                 size: int | tuple, format: pyv4l2.MetaFormat) -> None:
        super().__init__(vdev, mem_type, buf_type)
    ...
    @property
    def format(self) -> pyv4l2.MetaFormat:
        return self.__format
...
class MetaOutputStreamer(CaptureStreamer):
    def __init__(self, vdev: VideoDevice, mem_type: pyv4l2.MemType, buf_type: pyv4l2.BufType,
                 size: int, format: pyv4l2.MetaFormat) -> None:
        super().__init__(vdev, mem_type, buf_type)
    ...
    def reserve_buffers(self, num_bufs):
        ...
        for vbuf in self.vbuffers:
            ...
            fcntl.ioctl(self.fd, uapi.VIDIOC_QUERYBUF, v4l2buf, True)
...
class VideoBuffer:
    ...
    def __init__(self, mem_type: pyv4l2.MemType, index: int) -> None:
        self.index = index
        self.mem_type = mem_type
        # dmabuf fd
        self.fd = -1
